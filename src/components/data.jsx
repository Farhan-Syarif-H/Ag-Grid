export function getData() {
    return filesWithIcons;  // Kembalikan files yang sudah diberi ikon
}
...
const getFileIcon = (filename) => {
    ...
    return icons[extension] || "📄"; // Default icon file
};

// Fungsi untuk menambahkan ikon folder/file pada path
const addIconsToPaths = (files) => {
    return files.map(file => {
        const updatedPath = file.path.map((segment, index) => {
            if (index === file.path.length - 1 && segment.includes(".")) {
                // Jika ini adalah file (ada ekstensi), tambahkan ikon file
                return `${getFileIcon(segment)} ${segment}`;
            } else {
                // Jika ini adalah folder, tambahkan ikon folder
                return `📁 ${segment}`;
            }
        });
...
// Menambahkan ikon ke semua file dalam array
const filesWithIcons = addIconsToPaths(files);

// Contoh hasilnya
console.log(filesWithIcons);
